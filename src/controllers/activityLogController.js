...
async function getAllAfter(timestamp, limit) {
    return keysModel.pool.query('SELECT * from "activityLog" where timestamp <= to_timestamp($1) AND value > 0 ORDER BY timestamp desc LIMIT $2', [timestamp, limit]);
}

async function getAllAfterForBrightId(timestamp, limit, fromBrightId) {
    return keysModel.pool.query('SELECT * from "activityLog" where timestamp <= to_timestamp($1) AND value > 0 AND ("fromBrightId" = $3 OR "toBrightId" = $3) ORDER BY timestamp desc LIMIT $2', [timestamp, limit, fromBrightId]);
}
